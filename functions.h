...
void insert_m(tteacher *t){
    ...
    teacher_index ti;
    /*teacher_garbage tgarbage;
    fread(&tgarbage, sizeof(teacher_garbage), 1, mgarbage);
    if(!IsEmpty(mgarbage)){
        if(!tgarbage.isUsed){
            ti.teacher_id = tgarbage.teacher_id;
            ti.address = (tgarbage.teacher_id-1) * sizeof(teacher_index);
            fseek(mgarbage, (ti.teacher_id - 1) * sizeof(teacher_index), SEEK_SET);
            fwrite(&ti, );
        }
    }*/
